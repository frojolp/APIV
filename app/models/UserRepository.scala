...
@Singleton
class UserRepository @Inject()(dbConfigProvider: DatabaseConfigProvider)(implicit val executionContext: ExecutionContext) {
  ...
  private class UserTable(tag: Tag) extends Table[User](tag, "users") {
    ...
    override def * : ProvenShape[User] = (userid, vorname, nachname, geburtstag, telefonnummer, email) <> ((User.apply _).tupled, User.unapply)
  }
  ...
  def create(user: newUser): Future[User] = {
    val newUser = User(java.util.UUID.randomUUID(), user.vorname, user.nachname, user.geburtstag, user.telefonnummer, user.email)
